...
for cmd in lsblk findmnt wipefs sfdisk partprobe udevadm mkfs.vfat mkfs.btrfs btrfs bsdtar tar arch-chroot blkid install qemu-aarch64-static; do
  need "$cmd"
done
...
cleanup() {
  set +e
  mountpoint -q "$ROOT_MNT/dev/pts" && umount -l "$ROOT_MNT/dev/pts"
  mountpoint -q "$ROOT_MNT/dev/shm" && umount -l "$ROOT_MNT/dev/shm"
  mountpoint -q "$ROOT_MNT/dev" && umount -l "$ROOT_MNT/dev"
  mountpoint -q "$ROOT_MNT/proc" && umount -l "$ROOT_MNT/proc"
  mountpoint -q "$ROOT_MNT/sys" && umount -l "$ROOT_MNT/sys"
  mountpoint -q "$ROOT_MNT/boot" && umount "$ROOT_MNT/boot"
  mountpoint -q "$ROOT_MNT/.snapshots" && umount "$ROOT_MNT/.snapshots"
  mountpoint -q "$ROOT_MNT/home" && umount "$ROOT_MNT/home"
  mountpoint -q "$ROOT_MNT" && umount "$ROOT_MNT"
  mountpoint -q "$ROOT_MNT" && umount -R -l "$ROOT_MNT"
  rm -rf "$SUPPORT_DIR"
}
trap cleanup EXIT
...
mkfs.btrfs -f -O '^block-group-tree' -L ALARMROOT "$ROOT_PART"
...
mount -o subvol=@,compress=zstd:3,noatime "$ROOT_PART" "$ROOT_MNT"
mkdir -p "$ROOT_MNT/boot" "$ROOT_MNT/home" "$ROOT_MNT/.snapshots"
mount -o subvol=@home,compress=zstd:3,noatime "$ROOT_PART" "$ROOT_MNT/home"
mount -o subvol=@snapshots,compress=zstd:3,noatime "$ROOT_PART" "$ROOT_MNT/.snapshots"
mount "$BOOT_PART" "$ROOT_MNT/boot"

printf '\nExtracting Arch Linux ARM root filesystem...\n'
bsdtar -xpf "$TARBALL" -C "$ROOT_MNT"
mkdir -p "$ROOT_MNT/boot/archlinuxarm"
if compgen -G "$ROOT_MNT/boot/*" >/dev/null; then
  find "$ROOT_MNT/boot" -mindepth 1 -maxdepth 1 ! -name archlinuxarm -exec mv -t "$ROOT_MNT/boot/archlinuxarm" {} +
fi
...
cp -a "$SUPPORT_DIR/vendor/boot/firmware/." "$ROOT_MNT/boot/"
...
cp -a "$ROOT_MNT/boot/config.txt" "$ROOT_MNT/boot/config.txt.vendor-uconsole"
cp -a "$ROOT_MNT/boot/cmdline.txt" "$ROOT_MNT/boot/cmdline.txt.vendor-uconsole"
if ! grep -q '^initramfs initramfs-uconsole.img followkernel' "$ROOT_MNT/boot/config.txt"; then
  printf '\ninitramfs initramfs-uconsole.img followkernel\n' >> "$ROOT_MNT/boot/config.txt"
fi
printf 'console=serial0,115200 console=tty1 root=PARTUUID=%s rootfstype=btrfs rootflags=subvol=@ rw fsck.repair=yes rootwait cfg80211.ieee80211_regdom=HR\n' "$ROOT_PARTUUID" > "$ROOT_MNT/boot/cmdline.txt"

cat > "$ROOT_MNT/etc/fstab" <<'FSTAB'
LABEL=ALARMROOT / btrfs rw,noatime,compress=zstd:3,subvol=@ 0 0
LABEL=ALARMROOT /home btrfs rw,noatime,compress=zstd:3,subvol=@home 0 0
LABEL=ALARMROOT /.snapshots btrfs rw,noatime,compress=zstd:3,subvol=@snapshots 0 0
LABEL=ALARMBOOT /boot vfat defaults 0 0
FSTAB
printf 'arch-uconsole\n' > "$ROOT_MNT/etc/hostname"
printf 'KEYMAP=us\nFONT=ter-v16n\n' > "$ROOT_MNT/etc/vconsole.conf"

cp /usr/bin/qemu-aarch64-static "$ROOT_MNT/usr/bin/"
cp /etc/resolv.conf "$ROOT_MNT/etc/resolv.conf"
if ! grep -q '^DisableSandbox' "$ROOT_MNT/etc/pacman.conf"; then
  if grep -q '^#DisableSandbox' "$ROOT_MNT/etc/pacman.conf"; then
    sed -i '/^#DisableSandbox/s/^#//' "$ROOT_MNT/etc/pacman.conf"
  else
    sed -i '/^\\[options\\]/a DisableSandbox' "$ROOT_MNT/etc/pacman.conf"
  fi
fi
...
pacman -S --needed --noconfirm sudo networkmanager modemmanager openssh linux-firmware btrfs-progs mkinitcpio xorg-server xorg-xinit xorg-xrandr xorg-xsetroot xorg-xinput xterm alacritty lightdm lightdm-gtk-greeter i3-wm i3status rofi python python-gobject ttf-dejavu terminus-font vim nano git
...
depmod 6.12.62-v8+
cat > /etc/mkinitcpio-uconsole.conf <<'EOF'
MODULES=(btrfs xor raid6_pq)
BINARIES=()
FILES=()
HOOKS=(base udev modconf block filesystems fsck)
EOF
mkinitcpio -c /etc/mkinitcpio-uconsole.conf -k 6.12.62-v8+ -g /boot/initramfs-uconsole.img
...
sync
printf '\nPrepared %s for uConsole Arch Linux ARM with Btrfs root.\n' "$DEVICE"
